...
import { getMobilityProviderById, updateMobilityProviderById } from 'apiSdk/mobility-providers';
...
function MobilityProviderEditPage() {
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<MobilityProviderInterface>(
    () => (id ? `/mobility-providers/${id}` : null),
    () => getMobilityProviderById(id),
  );
  ...
  const handleSubmit = async (values: MobilityProviderInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateMobilityProviderById(id, values);
      mutate(updated);
      resetForm();
    } catch (error) {
      setFormError(error);
